    ...
    for book_url in category:
        ...
        if response.ok:
            ...
            for book in book_containers:
                link = book.a['href']
                links.append(url_base + 'catalogue/' + link.replace('../../../', '')) # To concatenate we have to replace
...
def csv_writer(data: list, category: str): # First we say that there will be two different parameter and what they are
    with open(csv_path + category + '.csv', 'w', newline='', encoding='utf-8-sig') as csvfile: # We, then, name the files with the list "category"
        fieldnames = ['productpage_url', 'upc', 'title', 'price_including_tax',  'price_excluding_tax', # Here we name all the columns in our CSV file
                      'number_available', 'product_description', 'category', 'review_rating', 'image_url']
        try: # Here we set what our CSV file will do in case of an error, restval is for the values that are not in any fields
            writer = csv.DictWriter(csvfile, fieldnames=fieldnames, restval='', extrasaction='raise')
            writer.writeheader()
            writer.writerows(data)
        except ValueError as error: # The extrasaction was set to raise to put this "ValueError" line in case of an error
